...
/**
 * Stores OAuth2 authorization request in a cookie instead of HTTP session.
 * Required when the OAuth2 flow goes through a reverse proxy (Vite dev server)
 * that doesn't preserve session cookies between the authorization request
 * and the callback redirect from Google.
 */
@Component
